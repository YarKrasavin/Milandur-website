...
<head>
  ...
  <title>Уникальный приватный сервер друзей</title>
  
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Ubuntu:ital,wght@0,400;0,500;1,500&family=Unbounded:wght@400;600;700&display=swap" rel="stylesheet">
  ...
  <style>
    ...
    * {
    ...
    /* 2. Шапка / Главный экран (Hero Section) */
    .hero {
      padding: 80px 0;
      display: grid;
    ...
    /* Контейнер картинки со свечением */
    .hero-image-wrap {
      position: relative;
      display: flex;
    ...
    .btn-primary {
      background: rgba(64, 134, 226, 0.25);
      border-color: rgba(64, 134, 226, 0.4);
    }

    .btn-primary:hover {
      background: rgba(64, 134, 226, 0.4);
      box-shadow: 0 0 20px rgba(64, 134, 226, 0.2);
    }

    /* Стили для дефолтной кнопки IP */
    .btn-secondary {
      background: rgba(0, 60, 255, 0.2);
      border-color: rgba(33, 45, 84, 0.6);
    }
    ...
    /* НОВЫЙ СТИЛЬ: Когда IP успешно скопирован */
    .btn-secondary.copied {
      background: rgba(46, 204, 113, 0.2);  /* Приятный зеленый оттенок */
  ...
  <script>
    function copyServerIP(buttonElement, ipAddress) {
      ...
      navigator.clipboard.writeText(ipAddress).then(() => {
        ...
        // Находим тег span внутри кликнутой кнопки, чтобы поменять текст
        const textSpan = buttonElement.querySelector('span');
        
        // Запоминаем старый текст («Скопировать IP»)
        const originalText = textSpan.innerText;
        
        // 2. Меняем текст и добавляем класс анимации цвета (зеленый)
        textSpan.innerText = 'Скопировано!';
        buttonElement.classList.add('copied');
        
        // Блокируем кнопку на время таймера, чтобы избежать лишних спам-кликов
        buttonElement.style.pointerEvents = 'none';

        // 3. Через 2.5 секунды возвращаем всё обратно
        setTimeout(() => {
